...
export class WorkHistoryComponent implements OnInit {
  ...
  constructor(private userService: UserService) { 
    ...
    this.form = new FormGroup({
      remoteWorkDays: new FormControl<number>(this.userService.user.remoteWorkHistory.remoteWorkDays, [
        Validators.required,
        Validators.pattern(/[1-5]/)
      ]),
      startDate: new FormControl<Date>(this.userService.user.remoteWorkHistory.startDate, [
        Validators.required,
        CustomValidatorsModule.minDateValidator(new Date(2010,0,1))
      ]),
      endDate: new FormControl<Date>(this.userService.user.remoteWorkHistory.endDate, [
        Validators.required,
  ...
  ngOnInit(): void {
    ...
    this.remoteDays = this.userService.user.remoteWorkHistory.remoteWorkDays;    
  }
