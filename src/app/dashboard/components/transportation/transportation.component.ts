...
export class TransportationComponent implements OnInit {  
  ...
  autoType: FormControl = new FormControl(this.userService.user.commute.autoType, [
    Validators.required
  ]);

  fuelType: FormControl = new FormControl(this.userService.user.commute.fuelType, [
    Validators.required
  ]);

  distanceToWork: FormControl = new FormControl(this.userService.user.commute.commuteDistancePerDay, [
    Validators.required,
    CustomValidatorsModule.minNumberValidator(1)      
  ]);

  publicCost: FormControl = new FormControl(this.userService.user.commute.publicTransportationCostPerDay, [
    Validators.required,
    CustomValidatorsModule.minNumberValidator(.01)                     
  ]);

  transportationMode: FormControl = new FormControl(this.userService.user.commute.transportationType, [
    Validators.required
  ]);

  timeToWork: FormControl = new FormControl(this.userService.user.commute.commuteMinutesPerDay, [
    Validators.required,
    CustomValidatorsModule.minNumberValidator(1)
  ]);
  ...
  ngOnInit(): void {    
    ...
    this.selectedTransportation = this.userService.user.commute.transportationType ?? "";
    this.milesToWork = this.userService.user.commute.commuteDistancePerDay;
    this.minutesToWork = this.userService.user.commute.commuteMinutesPerDay.toString();
    this.publicTransportationCost = this.userService.user.commute.publicTransportationCostPerDay?.toString() ?? "";    
  }
  ...
  private populateSelectFields(){
    ...
    for (let at in AutoType){
      this.autoTypes.push(at);
    }
